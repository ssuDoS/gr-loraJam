...
#ifndef INCLUDED_LORAJAM_FRAMEDETECT_IMPL_H
#define INCLUDED_LORAJAM_FRAMEDETECT_IMPL_H
...
class frameDetect_impl : public frameDetect
{
private:
      enum DecoderState
      {
      ...
      pmt::pmt_t out_dict;    // used for PMT dictionary sent on the output
      std::vector<gr_complex> samp_buf;   //// Samples buffer; used to contain the header symbols before finally decoding them
      uint32_t samp_buf_offset;           //// current offset of the samp buffer
      std::vector<gr_complex> cor_downchirp; //// CF0 corrected reference downchirp
      std::vector<gr_complex> cor_upchirp;   //// CFO_Int corrected reference upchirp
...
public:
    frameDetect_impl(uint32_t center_freq,
                     uint32_t bandwidth,
                     uint8_t sf,
...
#endif /* INCLUDED_LORAJAM_FRAMEDETECT_IMPL_H */
